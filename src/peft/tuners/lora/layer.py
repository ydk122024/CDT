...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        self.lora_route = nn.Linear(self.in_features, lora_nums, bias=False) 
        ...
        self.w_noise = nn.Parameter(torch.zeros(self.in_features, lora_nums), requires_grad=True)
        self.w_c = nn.Parameter(torch.Tensor(512, self.out_features))
        self.u = 0.1
        self.softplus = nn.Softplus()
        self.cluster_centers = torch.from_numpy(torch.load("CDT/clustering/cluster_centers_num32.pth")).to(dtype=torch.float16)
    ...
    def load_external_params(self):
        self.register_parameter('cluster_centers', self.cluster_centers)
        # self.register_parameter('covariances', self.covariances)
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters: 
                ...
                cluster = torch.matmul(self.cluster_centers, self.w_c) 
                hide_state = x.clone().mean(dim = 1)
                corresponding_cluster = []
                for i in range(hide_state.size(0)):
                    each_sample = hide_state[i:i+1, :]
              
                    
                    distances = torch.norm(cluster - each_sample, dim=1)
                    probabilities = torch.exp(-distances) / torch.sum(torch.exp(-distances))
                    if torch.isnan(probabilities).all():
                        
                        random_index = np.random.randint(0, len(cluster))

            
                        each_cluster = cluster[random_index:random_index+1, :]
                    else:
                
                        predicted_cluster = torch.argmax(probabilities).cpu().detach().numpy()
                        each_cluster = cluster[predicted_cluster-1:predicted_cluster, :]
    
                    corresponding_cluster.append(each_cluster)
                    
                corresponding_cluster = torch.cat(corresponding_cluster, dim=0).unsqueeze(1) 
                cluster_x = (1-self.u)*x + self.u*corresponding_cluster 
                raw_logits = self.lora_route(cluster_x) + (torch.randn_like(noise_stddev) * noise_stddev) 
                ...
                out = torch.einsum('bijk,bij->bijk', out, route_weight)  
                out = out.sum(dim=2)
