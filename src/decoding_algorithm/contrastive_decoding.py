...
class ContrastiveDecoding:
    ...
    def __init__(self, model_name, device="cuda", max_gpu_memory=39, amateur_adapter_path=None, master_adapter_path=None, base_expert_model_name = None, num_gpus=-1, amateur_model_nums_gpus=-1, master_model_nums_gpus=-1):
     
        self.model_name = model_name
        self.amateur_adapter = amateur_adapter_path
        self.master_adapter = master_adapter_path
        self.base_expert_name = base_expert_model_name
        self.device = device
        ...
        if self.amateur_adapter  is not None:
            
            self.amateur_model, self.amateur_model_tokenizer = self.load_lora_unmerged_model(self.base_expert_name, self.amateur_adapter, amateur_model_nums_gpus)  
        
 
            
            
        if self.master_adapter is not None:
            
            self.master_model, self.master_model_tokenizer = self.load_lora_unmerged_model(self.base_expert_name, self.master_adapter, master_model_nums_gpus) 

        else:
            self.master_model, self.master_model_tokenizer = self.load_model(self.base_expert_name, master_model_nums_gpus)
                   
            
        self.all_gpu_nums = num_gpus + amateur_model_nums_gpus + master_model_nums_gpus  
    ...
    def load_lora_unmerged_model(self, model_name, adapter_path, num_gpus, start_id=0):
    ...
    def generate(self, input_text=None, evil_input_text=None, input_ids=None, max_new_tokens=256, top_p=0.95, top_k=0, temperature=0.8, 
        ...
        with torch.no_grad():
            ...
            elif mode == "CDT-contrastive-decoding":
                outputs = self.model.generate(input_ids, max_length=max_len, num_return_sequences=1,
                    output_scores=True, return_dict_in_generate=True, contrastive_decoding=True, student_model=self.amateur_model, master_model=self.master_model, 
                    top_p=top_p, top_k=top_k, temperature=temperature, stopping_criteria=self.stopping_criteria, relative_top=relative_top, **kwargs)
    ...
    def lm_score(self, input_text1, input_text2, input_text3=None, pmi=False, max_new_tokens=256, top_p=0.95, top_k=0, temperature=0.8, mature_layer=None, premature_layer=None, candidate_premature_layers=[], mode='baseline', verbose=True, remove_stop_words=False, relative_top=0.1, relative_top_value=-1000.0, post_softmax=True, **kwargs):
        ...
        with torch.no_grad():
            ...
            if mode == 'baseline':
                ...
                if self.master_adapter is not None:
                    master_outputs = self.master_model(input_ids)[0].squeeze(0) 
                    master_logits = master_outputs.log_softmax(-1)[prefix_ids.shape[-1] - 1: -1, :] 
                    
                    diff_logits = base_logits + 0.1*master_logits - 2*amateur_logits
    ...
    def lm_prob(self, input_text1, input_text2, input_text3=None, pmi=False, max_new_tokens=256, top_p=0.95, top_k=0, temperature=0.8, mature_layer=None, premature_layer=None, candidate_premature_layers=[], mode='baseline', verbose=True, remove_stop_words=False, relative_top=0.1, relative_top_value=-1000.0, post_softmax=True, **kwargs):
        ...
        with torch.no_grad():
            ...
            if mode == 'baseline':
                ...
                diff_logits = base_logits + master_logits 
