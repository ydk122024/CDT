...
class GenerationMixin:
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
            ...
            if contrastive_decoding:  # Greedy CD
                ...
                if student_model is not None and master_model is not None:
                    # raise ValueError(
            ...
            if dola_decoding:  # Dola sample
                ...
                # assert student_model is not None or evil_input_ids is not None  # at least one mode is supported
                if student_model is not None or master_model is not None:
    ...
    def contrastive_greedy_decode(
        ...
        **model_kwargs,
        ...
        master_model_kwargs = copy.deepcopy(model_kwargs)
        ...
        while True:
            ...
            master_model_inputs = self.prepare_inputs_for_generation(input_ids, **master_model_kwargs)
            ...
            master_model_kwargs = self._update_model_kwargs_for_generation(
                master_outputs, master_model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            )
    ...
    def contrastive_decoding_sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            master_logits = master_outputs[0]
            final_logits = outputs[0]
            if relative_top > 0.0:
                final_logits = self.relative_top_filter(final_logits, relative_top)
                base_logits = base_logits.log_softmax(dim=-1)  
                master_logits = master_logits.log_softmax(dim=-1)
                mask = final_logits[0] < -1e3
                base_logits[0][mask] = -1e3   
                master_logits[0][mask] = -1e3
            logits = final_logits - base_logits +  2*master_logits 
            next_token_logits = logits[:, -1, :]
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if relative_top > 0.0:
                ...
                mask = final_logits[0] < -1e3
                base_logits[0][mask] = -1e3
            logits = final_logits - 0.5*base_logits   
            next_token_logits = logits[:, -1, :]
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # probs[torch.isnan(probs)] = 1e-8
            # probs[torch.isinf(probs)] = 1e-8
            # probs[probs < 0] = 1e-8
